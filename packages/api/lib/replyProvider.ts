...
export type ReplyProvider = 'openai' | 'groq'
...
export function resolveReplyProviderConfig(
  ...
  if (requestedProvider !== 'openai' && requestedProvider !== 'groq') {
    throw new Error(`PINCH_REPLY_PROVIDER must be "openai" or "groq", received "${requestedProvider}"`)
  }
  ...
  const model = cleanEnvironmentValue(environment.PINCH_REPLY_MODEL)
    || (provider === 'groq' ? DEFAULT_GROQ_REPLY_MODEL : DEFAULT_OPENAI_REPLY_MODEL)
  ...
  return {
    ...
    reasoningEffort,
    apiKey,
    baseURL: provider === 'groq' ? GROQ_OPENAI_BASE_URL : undefined,
  }
}
